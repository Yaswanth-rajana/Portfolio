...
export const ScrollStackItem = ({ children, itemClassName = '', variant = 'default' }) => {
  ...
  return (
    <div className={`scroll-stack-card ${variantClass} ${itemClassName}`.trim()}>{children}</div>
  );
};
...
const ScrollStack = ({
  ...
  const lastCardReachedRef = useRef(false);
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      ...
      stackCompletedRef.current = false;
      lastCardReachedRef.current = false;
      cardsRef.current = [];
  ...
  return (
    <div className={`scroll-stack-scroller ${className}`.trim()} ref={scrollerRef} style={{ backgroundColor: '#242424', padding: '2rem', borderRadius: '20px' }}>
